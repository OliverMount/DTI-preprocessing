...
MNE_path="/Users/heterobrainx/MATLABtoolboxes/MNE_MAC/bin"
...
if [ ! -d $Connectome ]; then
	...
	mkdir TD_matrix  # For storing Time delay matrix
...
if [[ ! -f TI.mif && ! -f T2.mif ]]; then
	echo "\n++ Converting  T1 T2 to mif files and moving to the DTI folder of $sub."
	mrconvert ../nifti/${sub}_T1.nii.gz T1.mif  # in the original orientation (not LAS))
	mrconvert ../nifti/${sub}_T2.nii.gz T2.mif
else
	echo "\n++  T1.mif and T2.mif already exists. Nothing to convert "
fi
